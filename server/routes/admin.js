...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    if (existingUser) return res.redirect('/admin?status=error&message=Username already exists');
...
router.post('/', async (req, res) => {
  ...
  try {
    ...
    if (!user) return res.redirect('/admin?status=error&message=User not found');

    const valid = await bcrypt.compare(password, user.password);
    if (!valid) return res.redirect('/admin?status=error&message=Invalid password');
...
router.post('/add-post', authMiddleware, upload.array('featuredMedia', 10), async (req, res) => {
  try {
    ...
    await Post.create({ title: req.body.title, body: req.body.body, media: mediaFiles });
